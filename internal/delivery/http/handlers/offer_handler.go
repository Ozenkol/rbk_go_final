...
func (h *OfferHandler) CreateOffer(c *gin.Context) {
	var req http_requests.CreateOfferRequest
	...
	res, err := h.deps.App.Commands.CreateOffer.Handle(c.Request.Context(), command.CreateOfferCommand{
		Offer: o,
	})
	if err != nil {
...
func (h *OfferHandler) UpdateOffer(c *gin.Context) {
	id := c.Param("id")
	var req http_requests.UpdateOfferRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	res, err := h.deps.App.Commands.UpdateOffer.Handle(c.Request.Context(), command.UpdateOfferCommand{
		Offer: o,
	})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
